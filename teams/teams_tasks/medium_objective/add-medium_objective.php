...
<?php
   if (isset($_POST['submit'])) {
     ...
     if ($db->run("INSERT INTO teams_medium_objectives(name, finish_date, teams_main_objectives_id) VALUES (?,?,?)", [$name, $finish_date, $_GET['id']])) {
...
<main>
  <?php
    $main_objective = $db->run("SELECT * FROM teams_main_objectives WHERE id = ?", [$_GET['id']])->fetch(PDO::FETCH_OBJ);
   ?>
   <?php if (false): ?>
